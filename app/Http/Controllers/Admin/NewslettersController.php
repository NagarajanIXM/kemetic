...
class NewslettersController extends Controller
{
    ...
    private function handleSendToAllNewsletters($title, $description)
    {
        ...
        // $newsletters = NewsletterTest::orderBy('created_at', 'desc')->get();
        ...
        foreach ($newsletters as $newsletter) {
            try {
                ...
                $sentCount++;
                sleep(2); // Add delay to avoid limits
            } catch (\Swift_TransportException $e) {
                // \Log::error("SMTP Error: " . $e->getMessage());
                break; // Stop sending if SMTP fails
            } catch (Exception $e) {
                // \Log::error("Email to {$newsletter->email} failed: " . $e->getMessage());
                continue; // Skip and move to the next email
            }
        }
        return $sentCount;

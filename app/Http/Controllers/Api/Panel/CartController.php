...
class CartController extends Controller
{
    ...
    public function createOrderAndOrderItemsOLD($carts, $calculate, $user, $discountCoupon = null)
    {
        if (!empty($discountCoupon)) {
            $totalCouponDiscount = $this->handleDiscountPrice($discountCoupon, $carts, $calculate['sub_total']);
        } else {
        ...
        foreach ($carts as $cart) {
            ...
            OrderItem::create([
                'user_id' => $user->id,
                'order_id' => $order->id,
                'product_id' => isset($product) ? ($product->id ?? $product) : null,
    ...
    public function checkoutOLD(Request $request)
    {
        ...
        if (!empty($carts) and !$carts->isEmpty()) {
            ...
            if (!empty($discountCoupon)) {
                $totalCouponDiscount = $this->handleDiscountPrice($discountCoupon, $carts, $calculate['sub_total']);
                // print_r($totalCouponDiscount);die;
                $calculate['total_discount'] += $totalCouponDiscount;
                $calculate["total"] = $calculate["total"] - $totalCouponDiscount;
            }

                ...
                <div class="course-content-body user-select-none">
                    ...
                    <div class="mt-35">
                        ...
                        <div class="tab-content" id="nav-tabContent">
                            <div class="mt-20 align-items-center d-flex flex-fill gap-2 justify-content-around">
                                <div class="d-flex align-items-center pointer">
                                    <i class="fas fa-thumbs-up interaction-icon{{ $course->id }} {{ $likeIconClass }}" data-type="like"></i>
                                    <span class="ml-1 font-14">{{ $totalStats->total_likes ?? 0 }}</span>
                                </div>
    
                                <div class="d-flex align-items-center">
                                    <i class="fas fa-eye text-primary" data-type="view"></i>
                                    <span class="ml-1 font-14 view-count-{{ $course->id }}">{{ $totalStats->total_views ?? 0 }}</span>
                                </div>
                        ...
                        <form action="/cart/store" method="post">
                            ...
                            <div class="mt-20 d-flex flex-column">
                                @if(!$canSale and $course->canJoinToWaitlist())
                                    <button type="button" data-slug="{{ $course->slug }}" class="btn1 btn btn-primary {{ (!$authUserJoinedWaitlist) ? ((!empty($authUser)) ? 'js-join-waitlist-user' : 'js-join-waitlist-guest') : 'disabled' }}" {{ $authUserJoinedWaitlist ? 'disabled' : '' }}>
                                ...
                                @elseif($hasBought or !empty($course->getInstallmentOrder()))
                                    <a href="{{ $course->getLearningPageUrl() }}" class=" btn2 btn btn-primary">{{ trans('update.go_to_learning_page') }}</a>
                                @elseif(!empty($course->price) and $course->price > 0)
                                    <button type="button" class="btn3 btn btn-primary {{ $canSale ? 'js-course-add-to-cart-btn' : ($course->cantSaleStatus($hasBought) .' disabled ') }}">
                                        @if(!$canSale)
                                ...
                                @else
                                    ...
                                    <a href="{{ $canSale ? '/course/'. $course->slug .'/free' : '#' }}" class="btn4 btn btn-primary {{ (! $canSale) ? (' disabled ' . $course->cantSaleStatus($hasBought)) : '' }}">
                                        @if(!$canSale)
                                            @if($course->checkCapacityReached())
                                                {{ trans('update.capacity_reached') }}
                                            @else
...
@push('scripts_bottom')
    ...
    <div class="modal fade" id="shareModal" tabindex="-1" role="dialog" aria-labelledby="shareModalLabel" aria-hidden="true">
     ...
     @include('web.default.blog.share_course_modal')
    <script>
        ...
        $(document).ready(function() {
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        let countSpan = $('.view-count-{{ $course->id }}');
                        countSpan.text(response.updated_views);
                    }
                }
            });
            
            $('.interaction-icon{{ $course->id }}').click(function() {
                let icon = $(this);
